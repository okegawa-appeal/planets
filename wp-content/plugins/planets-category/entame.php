<?php
/*
Plugin Name: Planets entame
Description: Planets entameのイベント表示アプリ
Version: 1.0
*/
...
class PL_ENTAME_EventList_Table extends WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        $data = $wpdb->get_results("SELECT * FROM $table_name WHERE type = 1 AND genre = 3 ORDER BY type,ord desc ", ARRAY_A);
        $this->items = $data;
    }
    ...
    function column_Control($item){
        ...
        $data .= '<br><a href="?page=planetsentameentry&edit=' . $item['id'] . '">編集</a>';
        if($item['slug']){
...
// Define custom WP_List_Table class for displaying custom table data
class PL_ENTAME_GoodsList_Table extends WP_List_Table
{
    // Define columns for the table
    function get_columns()
    {
    ...
    function prepare_items()
    {
        ...
        $data = $wpdb->get_results("SELECT * FROM $table_name WHERE type = 2 AND genre = 3 ORDER BY type,ord desc ", ARRAY_A);
        $this->items = $data;
    }
...
add_action('admin_menu', function(){
	...
	//---------------------------------	
    add_menu_page(
		'ENTAME TOP' // ページのタイトルタグ<title>に表示されるテキスト
		, 'ENTAME TOP'   // 左メニューとして表示されるテキスト
		, 'manage_options'       // 必要な権限 manage_options は通常 administrator のみに与えられた権限
		, 'planetsentameentry'        // 左メニューのスラッグ名 →URLのパラメータに使われる /wp-admin/admin.php?page=toriaezu_menu
		, 'planets_entame_entry_page_contents' // メニューページを表示する際に実行される関数(サブメニュー①の処理をする時はこの値は空にする)
...
// データを追加
function em_insert_data($title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url, $image,$ord,$type,$open,$category,$slug,$rate,$code) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'pl_event';
    $result = $wpdb->insert(
        $table_name,
...
// データを更新
function em_update_data($id, $title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url,$image,$ord,$type,$open,$category,$slug,$rate) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'pl_event';
    $result = $wpdb->update(
        $table_name,
...
//カテゴリフィルタ
function entame_top_filter($pieces){
    ...
    $pieces['where'] .= " AND t.term_id IN ( SELECT term_id FROM wp_termmeta WHERE meta_key = 'category-genre' AND meta_value = 'entame')"; 
    //echo "<pre>";
    //print_r($pieces);
    //echo "</pre>";
...
// サブメニューイベント表示
//=================================================
function planets_entame_entry_page_contents() {
	ob_start();
    // フォームからのデータを処理
    if (isset($_POST['action'])) {
        ...
        if ($action === 'create') {
            ...
            em_insert_data($title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url,$image,$ord,$type,$open,$category,$slug,$rate,$code);
            ...
            $open = intval($_POST['open']);
            $rate = intval($_POST['rate']);
            em_update_data($id, $title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url,$image,$ord,$type,$open,$category,$slug,$rate);
    ...
    if (isset($_GET['edit'])) {
        $id = intval($_GET['edit']);
        $data = em_read_data($id);
    }

    add_filter('terms_clauses', 'entame_top_filter', '', 1);
    ...
    EVENT
    <?php
        $event_table = new PL_ENTAME_EventList_Table();
        $event_table->prepare_items();
        $event_table->display();
        echo 'GOODS';
        $goods_table = new PL_ENTAME_GoodsList_Table();
