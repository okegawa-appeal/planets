<?php
/*
Plugin Name: Planets LUMISTAR
Description: Planets LUMISTARのイベント表示アプリ
Version: 1.0
*/
...
class PL_LUMISTAR_EventList_Table extends WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        $data = $wpdb->get_results("SELECT * FROM $table_name WHERE type = 1 AND genre = 2 ORDER BY type,ord desc ", ARRAY_A);
        $this->items = $data;
    }
    ...
    function column_Control($item){
        ...
        $data .= '<br><a href="?page=planetslumistarentry&edit=' . $item['id'] . '">編集</a>';
        if($item['slug']){
...
add_action('admin_menu', function(){
	...
	//---------------------------------	
    add_menu_page(
		'LUMISTAR TOP' // ページのタイトルタグ<title>に表示されるテキスト
		, 'LUMISTAR TOP'   // 左メニューとして表示されるテキスト
		, 'manage_options'       // 必要な権限 manage_options は通常 administrator のみに与えられた権限
		, 'planetslumistarentry'        // 左メニューのスラッグ名 →URLのパラメータに使われる /wp-admin/admin.php?page=toriaezu_menu
		, 'planets_lumistar_entry_page_contents' // メニューページを表示する際に実行される関数(サブメニュー①の処理をする時はこの値は空にする)
...
// データを追加
function lm_insert_data($title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url, $image,$ord,$type,$open,$category,$slug,$rate,$code) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'pl_event';
    $result = $wpdb->insert(
        $table_name,
...
// データを更新
function lm_update_data($id, $title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url,$image,$ord,$type,$open,$category,$slug,$rate) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'pl_event';
    $result = $wpdb->update(
        $table_name,
...
//カテゴリフィルタ
function lumistar_top_filter($pieces){
    ...
    $pieces['where'] .= " AND t.term_id IN ( SELECT term_id FROM wp_termmeta WHERE meta_key = 'category-genre' AND meta_value = 'lumistar')"; 
    //echo "<pre>";
    //print_r($pieces);
    //echo "</pre>";
...
// サブメニューイベント表示
//=================================================
function planets_lumistar_entry_page_contents() {
	ob_start();
    // フォームからのデータを処理
    if (isset($_POST['action'])) {
        ...
        if ($action === 'create') {
            ...
            lm_insert_data($title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url,$image,$ord,$type,$open,$category,$slug,$rate,$code);
            ...
            $open = intval($_POST['open']);
            $rate = intval($_POST['rate']);
            lm_update_data($id, $title,$talent,$reserve_start,$reserve_start_time,$reserve_end,$event_start, $url,$image,$ord,$type,$open,$category,$slug,$rate);
    ...
    add_filter('terms_clauses', 'lumistar_top_filter', '', 1);
    ...
    ARTIST
    <?php
        $event_table = new PL_LUMISTAR_EventList_Table();
